...
function formData(sectionValue, questionValue) {
  ...
  function parseForm(form_value)  {
    // window.alert("parseForm");
    var URLString = new String();

    var search_index = new Number();

    var index_string = new String();
    
    var old_url_value = new String();
    var new_url_value = new String();

    var end_slice_num = new Number();

    URLString = window.location.hash;

    search_index = URLString.indexOf($(form_value).attr("name"));

    new_url_value = $(form_value).attr("name") + "=" + $(form_value).val();
    
    // window.alert("new_url_value = " + new_url_value);


    if (search_index > -1) {
      if (URLString.indexOf("&", search_index) === -1)  {
        end_slice_num = URLString.length;
      } else  {
        end_slice_num = URLString.indexOf("&", search_index) + 1;
      }

      old_url_value = URLString.slice(search_index, end_slice_num);

      if ($(form_value).attr("name") === "stressor")  {

      }

      URLString = URLString.replace(old_url_value, new_url_value);
    } else  {
      URLString = URLString + "&" + new_url_value;
    }
  ...
  $(document).ready(
    function () {
      ...
      $("input[name='mood_type'], input[name='difficulty']").change(
        function () {
          window.alert("select");
          parseForm($(this));
        }
